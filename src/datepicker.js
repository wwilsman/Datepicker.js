...
class Datepicker {
  ...
  constructor(elem, opts) {
    ...
    if (typeof elem === 'string') {
      return transform($$(elem), (el) => new Datepicker(el, opts))
    }
    ...
    let queryResponsiveOpts = () => {
      ...
      queryTimeout = window.setTimeout(() => {
        this.set(queryMatched ? matchedOpts : origOpts)
    ...
    let isInput = this._isInput = (/input|textarea/i).test(elem.tagName)
    ...
    if (isInput) {
      ...
      elem.addEventListener('click', () => {
        if (hasClass(this.node, 'is-visible')) {
          this.hide()
        } else {
          this.open()
        }
    ...
    this.node.addEventListener('mouseup', (e) => {
      ...
      $$('[data-date].is-highlighted', this.container).forEach((el) => {
      ...
      if (mousedown && closest(e.target, '[data-date]', this.node)) {
        ...
        selection.forEach((d) => {
          $$('[data-date="' + d + '"]', this.container).forEach((el) => {
        ...
        if (!opts.multiple) {
          $$('[data-date].is-selected', this.container).forEach((el) => {
    ...
    this.node.addEventListener('click', (e) => {
      ...
      // clicked the year select but it hasn't been bound
        ...
        e.target.addEventListener('change', () => {
          ...
          let y = this._c.getFullYear()
          this._c.setFullYear(parseInt(e.target.value) - (c - y))
          this.draw()
        })
      ...
      // clicked the month select but it hasn't been bound
        ...
        e.target.dataset.bound = true
        e.target.addEventListener('change', () => {
          this._c.setMonth(e.target.value - e.target.dataset.cal)
          this.draw()
        })
      }
    }, false)
